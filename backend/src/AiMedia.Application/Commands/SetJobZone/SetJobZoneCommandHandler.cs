using AiMedia.Application.Interfaces;
using MediatR;
using Microsoft.EntityFrameworkCore;

namespace AiMedia.Application.Commands.SetJobZone;

public class SetJobZoneCommandHandler(IAppDbContext db)
    : IRequestHandler<SetJobZoneCommand>
{
    public async Task Handle(SetJobZoneCommand request, CancellationToken cancellationToken)
    {
        var job = await db.GenerationJobs
            .FirstOrDefaultAsync(j => j.Id == request.JobId && j.UserId == request.UserId, cancellationToken)
            ?? throw new InvalidOperationException("Job not found or access denied.");

        job.Zone = request.Zone;
        await db.SaveChangesAsync(cancellationToken);
    }
}
