    ...
    : IRequestHandler<ProcessPayPalWebhookCommand, bool>
{
    public async Task<bool> Handle(ProcessPayPalWebhookCommand request, CancellationToken ct)
    {
        ...
        if (!string.IsNullOrEmpty(request.WebhookId))
        {
            ...
            if (!valid)
            {
                logger.LogWarning("PayPal webhook signature verification failed");
                return false;
            }
        }
    ...
    private async Task<bool> HandleCompletedAsync(JsonElement resource, CancellationToken ct)
    {
        ...
        if (!TryParseCustomId(customId, out var userId, out var pack)) return false;
        ...
        if (user is not null)
            _ = email.SendReceiptEmailAsync(user.Email, user.FullName ?? "there", pack.Label, pack.Credits, pack.Price, captureId, ct);
    ...
    private async Task<bool> HandleDeclinedAsync(JsonElement resource, CancellationToken ct)
    {
        ...
        var user = await db.Users.FirstOrDefaultAsync(u => u.Id == userId, ct);
        if (user is not null)
            _ = email.SendPaymentDeclinedEmailAsync(user.Email, user.FullName ?? "there", ct);
    ...
    private async Task<bool> HandleReversedOrRefundedAsync(JsonElement resource, string reason, CancellationToken ct)
    {
        ...
        if (user is not null)
            _ = email.SendPaymentReversedEmailAsync(user.Email, user.FullName ?? "there", originalTx.Amount, ct);
