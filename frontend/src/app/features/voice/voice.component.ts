  ...
  <div class="w-[380px] flex-shrink-0 border-r border-border bg-white flex flex-col overflow-y-auto">
    ...
    <div class="px-5 py-4 border-t border-border space-y-3">
      <div class="flex items-center justify-between text-sm">
        ...
        <span class="font-semibold"><span class="text-accent">{{ costEstimate() }}</span> credits / 1K chars</span>
      </div>
      <button class="btn-primary w-full" (click)="generate()" [disabled]="!text.trim() || generating()">
...
export class VoiceComponent implements OnDestroy {
  ...
  tiers = [{ value: 'Free' as ModelTier, label: 'Kokoro' }, { value: 'Standard' as ModelTier, label: 'MiniMax' }];
  voices = [{ id: 'af_heart', name: 'Heart (Female)' }, { id: 'af_sky', name: 'Sky (Female)' }, { id: 'am_adam', name: 'Adam (Male)' }, { id: 'am_michael', name: 'Michael (Male)' }];
  ...
  costEstimate = () => estimateCredits('Voice', this.tier());

  generate() {
    if (!this.text.trim() || this.generating()) return;
    this.generating.set(true); this.jobStatus.set('Queued'); this.outputUrl.set(undefined); this.errorMsg.set(undefined);
    this.gen.generateVoice({ text: this.text, tier: this.tier(), voiceId: this.voiceId })
      .subscribe({
