...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, MediaPreviewComponent, JobStatusComponent, AspectRatioPickerComponent, DurationPickerComponent, ResolutionPickerComponent, ModelPickerComponent, AdvancedOptionsPanelComponent],
  template: `
  ...
  pickerGroups = computed<PickerGroup[]>(() =>
    this.modelGroups.map(g => ({
      ...
      models: g.subModels.map(m => ({
        ...
        creditsDisplay: this.pricingById().get(m.id) ?? (() => {
