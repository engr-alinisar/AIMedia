...
interface NavItem { label: string; route: string; icon: string; badge?: string; badgeColor?: string; }
...
<div class="flex h-screen overflow-hidden bg-surface">
  ...
  <aside class="fixed lg:static inset-y-0 left-0 z-50 w-64 lg:w-56 flex-shrink-0 bg-sidebar border-r border-border flex flex-col h-full overflow-y-auto transition-transform duration-300 lg:translate-x-0"
    ...
    <div class="px-4 py-5 flex items-center gap-2.5">
      ...
      <a routerLink="/" class="flex items-center gap-2.5">
        ...
        <div>
          <div style="font-size:15px;font-weight:700;color:#111827;line-height:1.1;">Ai<span style="color:#7C3AED;">Media</span></div>
  ...
  <div class="flex-1 flex flex-col overflow-hidden min-w-0">
    ...
    <header class="h-12 bg-white border-b border-border flex items-center px-3 lg:px-6 gap-2 lg:gap-4 flex-shrink-0">
      ...
      <span class="lg:hidden font-bold text-gray-900 text-sm flex-shrink-0">Ai<span style="color:#7C3AED;">Media</span></span>
              ...
              <div class="flex items-center justify-between px-4 py-3 border-b border-gray-100">
                <span class="text-sm font-semibold text-gray-900">
                  ...
                  @if (notif.unreadCount() > 0) {
                    <span class="ml-1.5 px-1.5 py-0.5 text-[10px] bg-red-500 text-white rounded-full font-bold">{{ notif.unreadCount() }}</span>
                  }
                </span>
                <div class="flex gap-3">
                  @if (notif.unreadCount() > 0) {
                    <button (click)="notif.markAllRead()" class="text-xs text-accent hover:underline">Mark all read</button>
                  }
                  @if (notif.notifications().length > 0) {
                    <button (click)="notif.clearAll()" class="text-xs text-gray-400 hover:text-red-500">Clear all</button>
                  }
                </div>
              </div>
...
export class PublicShellComponent implements OnInit, OnDestroy {
  ...
  private onDocumentClick = (e: MouseEvent) => {
    ...
    if (!document.querySelector('[data-bell-container]')?.contains(target)) this.showNotifications.set(false);
    if (!document.querySelector('[data-user-menu]')?.contains(target)) this.showUserMenu.set(false);
  };
